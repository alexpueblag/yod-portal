(function(root,factory){
  ...
  function badge(row){
    ...
    if(!row.url)return {cls:'soon',icon:'clock',text:'Próximamente'};
    if(s==='activo'&&safeUrl(row.url,row.system_id))return {cls:'active',icon:'shield-check',text:'Abrir'};
    if(s==='activo')return {cls:'invalid',icon:'alert-triangle',text:'Configuración inválida'};
    if(s==='mantenimiento')return {cls:'maintenance',icon:'shield-lock',text:'Mantenimiento · no disponible'};
    if(s==='revisión'||s==='revision')return {cls:'review',icon:'clipboard-check',text:'En validación · no disponible'};
    return {cls:'soon',icon:'alert-triangle',text:'No disponible'};
  }
  function enabled(row){return state(row.estado)==='activo'&&Boolean(safeUrl(row.url,row.system_id));}
  function card(row){
    var on=enabled(row),b=badge(row),id='portal-'+String(row.system_id||'modulo').replace(/[^a-z0-9_-]/gi,'-');
    var open=on?'<a class="card link active" href="'+esc(safeUrl(row.url,row.system_id))+'" data-system-id="'+esc(row.system_id)+'" aria-labelledby="'+id+'">':'<article class="card disabled state-'+esc(b.cls)+'" data-system-id="'+esc(row.system_id)+'" aria-labelledby="'+id+'">';
  ...
  return Object.freeze({destinations:DESTINATIONS,trackDestinations:TRACK_DESTINATIONS,state:state,safeUrl:safeUrl,safeTrackUrl:safeTrackUrl,enabled:enabled,badge:badge,card:card,cleanRows:cleanRows,escapeHtml:esc});
});
